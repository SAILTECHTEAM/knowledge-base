...
This article clarifies the measurement system and maps metric values back onto the core components of detection architectures, as dissected in [Deconstructing Object Detection](./deconstruction/architecture-overview).

## What detection tasks need measured
...
1. **Localization accuracy**: how tightly predicted boxes hug true object geometric borders. This dimension directly tests detection heads' box parameterization and regression-loss design; see [box representation and regression loss](./deconstruction/bbox-regression-loss).
2. **Classification correctness**: the purity of semantic recognition inside candidate regions and the calibration quality of classification confidence. This dimension maps to classification losses and confidence design in [classification confidence and detection heads](./deconstruction/classification-head).
3. **Duplicate suppression**: whether the model emits multiple redundant boxes for one true object. This performance answers directly to training-time sample matching plus inference-time non-maximum suppression (NMS); see [label assignment](./deconstruction/label-assignment).
4. **Multi-scale and dense-layout completeness**: whether the model detects everything without omission when huge and tiny objects coexist or small objects pack densely. This dimension mainly tests multi-scale pyramid semantic scheduling; see [multi-scale features and neck architectures](./deconstruction/neck-architecture).
...
## Hit rules, matching protocols, and averaging
...
### IoU and single-box hit decisions
...
### The essential evaluation-versus-training matching difference

Beginners easily conflate evaluation-stage IoU matching with training-stage [label assignment](./deconstruction/label-assignment). Both compute geometric overlap with IoU, but their stages, purposes, and permitted operations differ completely:
...
```mermaid
flowchart TD
    A["All inference-emitted prediction boxes"] -->|"grouped by class, dataset-wide confidence-descending sort"| B["High-confidence box"]
    B --> C{"Find max-IoU ground-truth box"}
    C -->|"IoU >= threshold and truth unclaimed"| TP["TP — truth marked claimed"]
    C -->|"IoU >= threshold but truth claimed"| FPD["FP — duplicate detection"]
    C -->|"IoU &lt; threshold"| FPL["FP — localization miss or background error"]
    D["Truth boxes matched by no prediction"] --> FN["FN — missed detection"]
```
...
### Average precision and multi-threshold averaging
...
:::caution[Misleading case]
with both models at 85% VOC AP, readers cannot tell whether one's boxes merely graze objects (IoU=0.51) or perfectly hug contours (IoU=0.90). Reading only this metric overvalues coarse-localization models' deployment worth.
:::
...
:::caution[Misleading case]
multi-threshold averaging smooths extreme-range differences. A model with sky-high low-threshold recall but weak high-threshold behavior can tie another emitting few high-quality precise boxes — while the two behave completely differently in production.
:::
...
## Size splits, recall, long tails, and speed costs
...
### Object-size split metrics ($AP_S, AP_M, AP_L$)
...
:::caution[Misleading case]
reading only $AP_S$ hides context-modeling defects on large-scale continuous structures; annotation noise (slight human-box drift crashing IoU) also swings small-object metrics far harder than large ones.
:::
...
:::caution[Misleading case]
large objects are usually few or semantically salient in datasets, so high $AP_L$ comes easily — easily masking severe misses under dense occlusion and small-object scenes.
:::

### Average recall and max-detection caps ($AR_{\text{max}}$)
...
### Long-tail and open-vocabulary evaluation
...
### Deployment cost metrics: latency, parameters, FLOPs
...
:::caution[Misleading case]
latency numbers detached from hardware, batch size, and pre/post-processing inclusion are incomparable; reporting backbone-forward time only while hiding NMS time severely underestimates one-to-many dense detectors' real running bottleneck.
:::
...
## Error splitting and attribution diagnosis
...
- **Attribution**: classification branches fired successfully, but regression branches drifted geometrically. Check [box representation and regression loss](./deconstruction/bbox-regression-loss) first for coordinate-loss forms (whether Smooth L1 lacks border gradient sensitivity, whether Distribution Focal Loss is needed) and feature-sampling receptive-field versus geometric misalignment (coarse RoIPool quantization to RoIAlign).

### Classification-confusion error ($E_{\text{cls}}$)

- **Physical symptom**: the box overlaps truth well geometrically ($\text{IoU} \ge 0.50$) but carries the wrong class label.
- **Attribution**: geometry works, high-level semantic discrimination distorts. Check [classification confidence and detection heads](./deconstruction/classification-head) first for classification-loss design (class imbalance collapsing rare classes into common ones) and missing head decoupling (classification and regression features forcibly sharing one kernel causing task conflict).

### Duplicate-detection error ($E_{\text{dup}}$)
...
With the measurement chain's logic mastered, ablation tables in papers reveal each model part's true behavior in geometric regression, feature fusion, and sample assignment, closing the technical loop between measurement logic and component design.
