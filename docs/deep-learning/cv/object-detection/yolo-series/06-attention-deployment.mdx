...
Setting $l = 4$ reduces the quadratic term to **$\frac{1}{4}$** of its original cost, while each partition retains a receptive field covering one-quarter of the full feature map. The cost of this design: single-layer self-attention confined within each segment lost cross-region global interaction, leaving cross-region information flow to be recovered gradually by deeper layers. The authors accepted this compromise to bring quadratic attention complexity down to real-time-compatible levels.
...
Architecturally, YOLOv12's core trade-off localized the globality of self-attention. The authors gave up unrestricted global multi-head attention, accepting within-segment interaction plus large-kernel convolutional position sensing in exchange for real-time forward speed on arbitrary-aspect-ratio inputs and stable training.
